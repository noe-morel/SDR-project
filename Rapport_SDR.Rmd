...
```{r 1 jeune 1 solution Regions Normalise, fig.width=14}
...
Enfin, nous nous sommes intéressés à comment réaliser un dashboard interactif. L'objectif était simple : permettre à l'utilisateur de naviguer dans l'entièreté de nos donnés et de lui donner accès à la meilleure expérience utilisateur possible.

Pour celà, il falait donc que les graphiques associés aux données soient affichés en temps réel en fonction des choix de l'utilisateur. Nous avons rapidement choisi de partir sur **Shiny** qui semblait être une manière simple de permettre à un utilisateur de naviguer parmi les données. Shiny est une librairie qui permet la création de pages web interactives sur lesquelles il est possible de réaliser toutes les analyses et actions que l'on aurait pu faire sur R autrement. Ici, nous nous intéressons seulement à la partie intéractive et nous faisons donc une page web qui ne sera utilisée qu'en local. Nous avons aussi utilisé **Ploty** qui est compatible avec Shiny et permet d'intéragir avec les graphiques afin d'avoir accès aux données dans les graphiques et de les manipuler.

![Résultat du Dashboard interactif](dashboard.png)
Ceci est le résultat auquel nous sommes arrivés sur notre dashboard. A gauche, on voit les différents choix que peut faire l'utilisateur. Il peut choisir un indicateur parmi les 58 qui sont disponibles, et ce choix sera directement reflété sur la partie de droite. Cette partie de droite comprend une évolution au niveau national, avec les points de données en fonction du temps et une regression dont on peut aussi choisir la méthode à gauche, une carte présentant les derniers résultats disponibles temporellement au niveau départemental, et des précisions sur l'indicateur choisi avec la mesure associée, un intitulé plus long de la mesure et l'unité des données.

Cette combinaison permet d'avoir une prise d'information très rapide et synthétique sur les données. En effet, on peut à la fois voir la tendance (quand il y en a une) au niveau national et l'expression des différences au niveau territorial. Par exemple, on peut voir des différences très marqués sur le territoire, avec l'exemple le plus marqué étant les données sur l'agriculture où on n'a pas de données dans Paris, sans trop de surprise.

Enfin, dans un but d'accessibilité, nous avons ajouté à l'utilisateur la possibilité de choisir les couleurs qui forment le gradient sur la carte des départements. En effet, pour que ça puisse être adapté aux sensibilités et aux handicaps de tous, l'utilisateur peut modifier les couleurs. Pour celà, nous avons utilisé une petite librairie qui est un complément à Shiny, **colourpicker**, et qui permet de choisir librement une couleur. 

![Choix des couleurs avec colourpicker](colourpicker.png)
...
En conclusion de ce projet de visualisation de données axé sur l'impact des mesures gouvernementales, il est indéniable que l'exploration de la base de données a été fascinante et révélatrice. L'analyse des chiffres a permis de mettre en lumière les résultats globalement positifs des mesures gouvernementales mises en place. Cependant, il est crucial de souligner la nécessité de relativiser ces chiffres, étant donné qu'ils proviennent de sources gouvernementales.

Le caractère instructif de ce projet réside dans sa capacité à rendre les informations complexes accessibles à un large public grâce à des visualisations claires et compréhensibles. Les diverses représentations graphiques ont joué un rôle essentiel dans la communication des résultats, permettant une interprétation visuelle qui transcende les barrières techniques.

Il est à noter que bien que les chiffres témoignent d'une amélioration générale, il est essentiel de garder à l'esprit le contexte dans lequel ces données ont été collectées. L'objectivité et la transparence doivent demeurer au cœur de toute analyse, surtout lorsque les données émanent directement du gouvernement. Les conclusions tirées de ce rapport soulignent la nécessité d'une approche critique et d'une prise de recul vis-à-vis des données fournies.

En définitive, ce projet de visualisation de données a constitué une expérience enrichissante, mettant en évidence l'importance cruciale d'une compréhension approfondie des données sous-jacentes. Les leçons tirées de cette étude contribuent non seulement à notre compréhension des impacts des mesures gouvernementales, mais renforcent également l'importance de la transparence et de la vérification indépendante dans l'évaluation de telles initiatives.
>>>>>>> origin/main
